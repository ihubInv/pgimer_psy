...
const authenticateToken = async (req, res, next) => {
  try {
    ...
    if (MobileToken.isMobileTokenString(token)) {
      try {
        ...
        const userResult = await db.query(
          'SELECT id, name, role, department, email, is_active FROM users WHERE id = $1',
    ...
    try {
      ...
      'SELECT id, name, role, department, email, is_active FROM users WHERE id = $1 AND email = $2',
  ...
  try {
    ...
    if (token) {
      try {
        ...
        const userResult = await db.query(
          'SELECT id, name, role, department, email FROM users WHERE id = $1 AND email = $2',
          [decoded.userId, decoded.email]
        );
