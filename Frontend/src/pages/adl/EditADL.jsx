...
import { useGetADLFileByIdQuery, useGetADLFileByPatientIdQuery, useUpdateADLFileMutation, useCreateADLFileMutation } from '../../features/adl/adlApiSlice';
...
const EditADL = ({ adlFileId, isEmbedded = false, patientId: propPatientId = null, clinicalProformaId: propClinicalProformaId = null, readOnly = false, initialAdlData = null }) => {
  ...
  // For regular ADL file ID routes (/adl/:id)
  const { data: adlData, isLoading: isLoadingADLById } = useGetADLFileByIdQuery(
    isPatientRoute ? undefined : id, 
    { skip: !id || isPatientRoute || isNewRoute }
  );
  
  // For patient route (/adl/patient/:id), we need to find ADL by patient_id
  const { data: adlByPatientData, isLoading: isLoadingADLByPatient } = useGetADLFileByPatientIdQuery(
    isPatientRoute ? id : undefined,
    { skip: !id || !isPatientRoute }
  );
  
  // Combine loading states
  const isLoadingADL = isLoadingADLById || isLoadingADLByPatient;
  
  // Handle different possible API response structures
  // Backend returns: { success: true, data: { adlFile: ... } } for single file
  // Backend returns: { success: true, data: { adlFiles: [...] } } for patient route
  const adlFileFromId = adlData?.data?.adlFile || adlData?.data?.adl_file || adlData?.data?.file || adlData?.data;
  
  // For patient route, get the most recent ADL file (first in the array - sorted by updated_at desc)
  const adlFilesFromPatient = adlByPatientData?.data?.adlFiles || [];
  const adlFileFromPatient = adlFilesFromPatient.length > 0 ? adlFilesFromPatient[0] : null;
  
  // Use the appropriate ADL file based on route type
  const adlFile = isPatientRoute ? adlFileFromPatient : adlFileFromId;
  ...
  // Update mode: 
  //   - mode === 'update' (explicit)
  //   - Regular route: id exists AND adlFile exists
  //   - Patient route: adlFile exists (found existing ADL for patient)
  // Create mode: no adlFile found OR mode === 'create' OR isNewRoute
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      } else if (adlFile && (adlFile.id || id)) {
        // Update existing ADL file
        // IMPORTANT: Use adlFile.id for the update (not `id` which could be patient_id on patient routes)
        const adlFileId = adlFile.id || parseInt(id, 10);
        const updateData = {
          id: adlFileId,
          ...prepareDataForSubmission(formData)
        };
      ...
      <div className="relative z-10 w-full px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
        ...
        <Card className="relative shadow-2xl border border-white/40 bg-white/70 backdrop-blur-2xl rounded-3xl overflow-hidden mb-6">
          ...
          {expandedCards.mainWrapper && (
            <div className="p-6">
              <form onSubmit={handleSubmit} className="space-y-6">
                {/* Render all form sections using renderFormContent for consistency */}
                {renderFormContent()}
