...
const ClinicalProformaDetails = ({ proforma: propProforma }) => {
  ...
  const handlePrintPatientDetails = () => {
    ...
    printWindow.document.write(generatePatientDetailsPrintHtml(patient, proforma, null));
  ...
  const handlePrintClinicalProforma = () => {
    const printWindow = window.open('', '_blank');
    if (!printWindow) { toast.error('Please allow pop-ups to print this section'); return; }
    printWindow.document.write(generateClinicalProformaPrintHtml(proforma, patient, clinicalOptions));
    printWindow.document.close();
    printWindow.onload = () => setTimeout(() => { printWindow.print(); toast.success('Print dialog opened'); }, 400);
  };
  ...
  const handlePrintADL = () => {
    if (!adlFile) {
      toast.error('Intake record data is not loaded yet');
      return;
    }
    ...
    const printContent = generateAdlIntakePrintHtml(adlFile, patient);
